...
{
    // Ensure SAI kernel clocks are sourced from PLL3 so HAL can compute
    // correct dividers for 96 kHz / 192 kHz audio rates.
    static bool sai_clock_configured = false;
    if(!sai_clock_configured)
    {
        RCC_PeriphCLKInitTypeDef PeriphClkInitStruct = {0};
        PeriphClkInitStruct.PeriphClockSelection = RCC_PERIPHCLK_SAI1 | RCC_PERIPHCLK_SAI23;
        PeriphClkInitStruct.Sai1ClockSelection   = RCC_SAI1CLKSOURCE_PLL3;
        PeriphClkInitStruct.Sai23ClockSelection  = RCC_SAI23CLKSOURCE_PLL3;

        // PLL3 config (matches DaisyDuino's System clock defaults)
